...
class Wxpay_client_pub extends Common_util_pub 
{
	...
	function postXml()
	{
		//$log = new Log_();
	    $xml = $this->createXml();
		//$log->log_result('test11.log',$xml);
		$this->response = $this->postXmlCurl($xml,$this->url,$this->curl_timeout);
	
		//$log->log_result('test11.log',$this->response);
...
{	
	function __construct($pay_config) 
	{
		...
		$this->appid = $pay_config['weixin_appid'];
		$this->account = $pay_config['weixin_account'];
		$this->appsecret = $pay_config['weixin_appsecret'];
		$this->key = $pay_config['weixin_key'];		
	}
	...
	function createXml()
	{
		//echo '<pre>';print_r($this->parameters);
		try
		{
			//检测必填参数
			if($this->parameters["out_trade_no"] == null) 
			{
		   	...
		   	$this->parameters["mch_id"] = $this->account;//商户号
