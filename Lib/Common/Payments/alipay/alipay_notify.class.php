...
class AlipayNotify {
     ...
     * @return 验证结果
     */
	function verifyNotify($result = array()){
		$data = empty($result) ? $_POST : $result;
		if(empty($data)) {//判断POST来的数组是否为空
			...
			//生成签名结果
			$isSign = $this->getSignVeryfy($decrypt_post_para, $data["sign"],false);
     ...
     * @return 验证结果
     */
	function verifyReturn($result = array()){
		$data = empty($result) ? $_GET : $result;
		if(empty($data)) {//判断GET来的数组是否为空
			return false;
		}
		else {
			//生成签名结果
			$isSign = $this->getSignVeryfy($data, $data["sign"],true);
     ...
     * @return 验证结果
     */
	function OldverifyReturn($result = array()){
		$data = empty($result) ? $_GET : $result;
		if(empty($data)) {//判断POST来的数组是否为空
			return false;
		}
     ...
     * @return 验证结果
     */
	function OldverifyNotify($result=array()){
		$data = empty($result) ? $_POST : $result;
		if(empty($data)) {//判断POST来的数组是否为空
			return false;
		}
