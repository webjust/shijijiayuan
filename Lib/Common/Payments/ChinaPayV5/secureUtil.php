            ...
            if (is_file($filePath)) {
                if (pathinfo($file, PATHINFO_EXTENSION) == 'cer') {
                    if (getCertIdByCerPath($filePath) == $certId) {
                        ...
                        return getPublicKeyv5($filePath);
                    }
                }
            }
        }
 ...
 * @return unknown
 */
function getCertIdv5($cert_path)
{
...
{
    // 签名证书路径
    return getCertIdv5(SDK_SIGN_CERT_PATH);
}
 ...
 * @return string
 */
function getPublicKeyv5($cert_path)
{
...
function encryptPan($pan)
{
    ...
    $public_key = getPublicKeyv5($cert_path);
