...
//        echo "<pre>";print_r($model);exit;
    ...
    static public function getAccessList($authId) {
        ...
        // Db方式权限数据
		$center = str_replace("mysql://","",C('DB_CUSTOM'));
		...
		$obj_stmt = $pdo_conn->prepare($sql);
		$pdo_conn->query("set names utf8");
		$obj_stmt->setFetchMode(PDO::FETCH_ASSOC);
		if(!$obj_stmt->execute()){
			die("无法获取此域名的用户授权信息。");
		}
		$apps =  $obj_stmt->fetchAll();
		/**
		...
		**/
        ...
        foreach ($authoritys['no'] as $module_name => $action) {
            ...
            if(!empty($action) && is_array($action)){
                foreach($action as $key=>$vals){
                    $o_action_name = strtoupper($key);
