<?php
// +----------------------------------------------------------------------
// | Copyright (c) Copyright (C) 2013, Shanghai GuanYiSoft Co., Ltd.
// +----------------------------------------------------------------------
...
class Image {
    ...
    function showImg($imgFile, $text='', $x='10', $y='10', $alpha='50',$position=9,$font_size=12) {
        //获取图像文件信息
            ...
            if ($im) {
                ...
                if (!empty($text)) {
                    ...
                    if (is_file($text) && file_exists($text)) {//判断$text是否是图片路径
                        ...
                        $fontarea=ImageTTFBBox(20, 0, './Public/Uploads/simhei.ttf', $text);
                        $font_w=$fontarea[2]-$fontarea[0]+15;
                        $font_h=$fontarea[1]-$fontarea[7];
                        if($position>9 || $position <1){
                            $position = 9;
                        }
                        //左上角
                        ...
                        //上居中
                        if($position == 2){
                            $posX = ( $info["width"] - $font_w ) / 2 ;
                            $posY=$font_h;
                        }
                        //右上角
                        if($position ==3){  
                            $posX = ( $info["width"] - $font_w );
                            $posY=$font_h;
                        }
                        //左居中
                        if($position ==4){  
                             $posX=0;
                             $posY=($info["height"] - $font_h)/2;
                        }
                        //中部
                        if($position ==5){   
                            $posX = ( $info["width"] - $font_w ) / 2 ;
                        ...
                        ImageTTFText($im,$font_size,0,$posX,$posY,$textcolor,'./Public/Uploads/simhei.ttf',$text);
                    }
                    //ImageDestroy($tc);
                }
                //水印结束
