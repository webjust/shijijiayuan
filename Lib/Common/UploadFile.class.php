...
class UploadFile {//类定义开始
     ...
     * @return string
     */
    public function upload($savePath ='',$tmp_file = array()) {
        //如果不指定保存文件名，则由系统默认
        if(empty($savePath))
        ...
        // 获取上传的文件信息
        // 对$_FILES数组信息处理
        if(is_array($tmp_file) && !empty($tmp_file)){
            $files	 =	 $this->dealFiles($tmp_file);
        }else{
            $files	 =	 $this->dealFiles($_FILES);
        }
       // writeLog(var_export($files,1),"uploadfile.log");
        ...
        if($isUpload) {
			...
			if(!empty($_SESSION['OSS']['GY_OSS_PIC_URL']) || (!empty($_SESSION['OSS']['GY_OTHER_IP']) && !empty($_SESSION['OSS']['GY_OTHER_ON']) )){
				...
				foreach($fileInfo as $val){
					if(!empty($val['savepath'])){
						$ary_request_data['upfile']="@".$_SERVER['DOCUMENT_ROOT'].trim($val['savepath'],".").$val['savename'];
						$ary_request_data['file_path']=trim($val['savepath'],".").$val['savename'];
						break;
					}
				}
			...
			if($_SESSION['OSS']['GY_QN_ON'] == '1'){
				foreach($fileInfo as $val){
					$file_path = $_SERVER['DOCUMENT_ROOT'].trim($val['savepath'],".").$val['savename'];
					$file['tmp_name'] = $file_path;
					$key = trim($val['savepath'],".").$val['savename'];
					$setting['key'] = $key;
