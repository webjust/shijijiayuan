<?php
	class QiniuStorage {
		 ...
		 *获取某个路径下的文件列表
		 *@param $prefix     列举前缀
		 *@param $marker     列举标识符上一次列举返回的位置标记，作为本次列举的起点信息。缺省值为空字符串。
		 *@param $limit      单次列举个数限制
		 *@param $delimiter  指定目录分隔符
		**/
		
		public function getList($ary_query = array()){
			//$query = array_merge(array('bucket'=>$this->bucket), $query);
			$query = array('bucket'=>$this->bucket);
			if (!empty($ary_query['prefix'])) {
				$query['prefix'] = $ary_query['prefix'];
			}else{
				$query['prefix'] = '/Public/Uploads/'.CI_SN.'/';
			}
			if (!empty($ary_query['marker'])) {
				$query['marker'] = $ary_query['marker'];
			}
			if (!empty($ary_query['limit'])) {
				$query['limit'] = $ary_query['limit'];
			}else{
				$query['limit'] = 100;
			}
			if (!empty($ary_query['delimiter'])) {
				$query['delimiter'] = $ary_query['delimiter'];
			}
		...
		public function getInfo($key){
			$key = trim($key);
			$url = "{$this->QINIU_RS_HOST}/imageInfo/" . self::Qiniu_Encode("{$this->bucket}:{$key}");
			$accessToken = $this->accessToken($url);
			$response = $this->request($url, 'POST', array(
				'Authorization'=>"QBox $accessToken",
			));
