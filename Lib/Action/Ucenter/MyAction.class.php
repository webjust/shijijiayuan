...
class MyAction extends CommonAction {
    ...
    public function index() {
        $this->redirect(U('Ucenter/My/pageProfile'));
    }
       ...
       // if($res_del){
        ...
        if ($ary_result && $ary_res['result']) {
            ...
            if($PointCfg['is_consumed'] == 1){
                ...
                $fieldWhere = array();
                $fieldWhere['u_id'] = $m_id;
                $fieldWhere['mfi.is_need'] = 0;
                $fieldWhere['mfi.is_display'] = 1;
                $fieldWhere['mfi.is_edit'] = 1;
                $fieldWhere['mfi.is_status'] = 1;
                $fieldWhere['mfi.filelds_point'] = array('neq',0);
                //搜索出会员其他属性项有设置积分
                $ary_point_fields=M('MembersFieldsInfo mfi',C('DB_PREFIX'),'DB_CUSTOM')
                    ->join(C('DB_PREFIX').'members_fields mf on(id = field_id)')
                    ->where($fieldWhere)
                    ->select();
            ...
            $this->success('修改成功', U('Ucenter/My/pageProfile'));
        } else {
...
//        echo "<pre>";print_r($html);exit;
    ...
    public function doChange() {
        ...
        if (strlen($params['new_m_password']) < 6 || strlen($params['new_m_password']) > 16) {
            $ary_res['msg'] = '密码至少6位且不能大于16位...';
            $this->ajaxReturn(array('data' => $ary_res['data'], 'msg' => $ary_res['msg'], 'success' => $ary_res['success']));
        }
    ...
    public function pageDeliver() {
        ...
        if (!empty($raid) && (int) $raid > 0) {
            //根据收获ID，获取对应相关数据
            $ary_racity = $city->getReceivingAddress('', $raid);
        ...
        if($is_foreign['GY_IS_FOREIGN']['sc_value'] == 1){
            $ary_addr = $city->getReceivingAddressPage($_SESSION ['Members'] ['m_id']);
            $ary_deliver = $ary_addr['addr'];
        }else{
            $ary_deliver = $city->getReceivingAddress($member['m_id']);
        }
        foreach($ary_deliver as &$val){
            if($val['ra_mobile_phone']){
                $val['ra_mobile_phone'] = vagueMobile(($val['ra_mobile_phone']));
            }
        }
        $empty_addr = '';
        if (empty($ary_deliver) && !is_array($ary_deliver)) {
            $empty_addr .= "<tr><td colspan='6'>暂无数据</td></tr>";
        }
		$is_foreign = D('SysConfig')->getCfg('GY_SHOP','GY_IS_FOREIGN');

        $this->assign($is_foreign);
        $this->assign("empty", $empty_addr);
        $this->assign("citys", $ary_city);
        $this->assign("member", $member);
        $this->assign("deliver", $ary_deliver);
        $this->assign($is_foreign);
        $this->assign('page', $ary_addr['pageHtml']);
    ...
    public function doDelDeliver() {
        ...
        if (empty($ary_res) && (int) $ra_id <= 0) {
            ...
            if ($ary_result) {
                $this->success('删除成功', array('确认' => U('Ucenter/My/pageDeliver')));
                //$ary_res['msg'] = "删除成功";
                //$ary_res['success'] = "1";
                //$ary_res['data'] = $ra_id;
            } else {
    ...
    public function doAddDeliver() {
        ...
        $params = array(
            ...
            'ra_id_card' => $this->_post('ra_id_card'),
       ...
       /* if($params['ra_mobile_phone'] && !strpos($params['ra_mobile_phone'],'*')){
            $params['ra_mobile_phone'] = encrypt2($params['ra_mobile_phone']);
        }*/
        /*if(strpos($params['ra_mobile_phone'],'*')){
            unset($params['ra_mobile_phone']);
        }*/
        if (!empty($raid) && (int) $raid > 0) {
            $params['ra_id'] = $raid;
...
//        echo "<pre>";print_r($lists);exit;
        foreach ($lists as &$list) {
			...
			if(!empty($list['file_url'])){
				$list['file_url'] = D('QnPic')->picToQn($list['file_url']);
			}
        }
    ...
    public function pageInvoice() {
        ...
        foreach($ary_invoice_collect as &$val){
            $val['invoice_phone'] = decrypt($val['invoice_phone']);
            $RegExp  = "/^((13[0-9])|147|(15[0-35-9])|180|182|(18[5-9]))[0-9]{8}$/A";
            if(preg_match($RegExp,$val['invoice_phone'])){
                $val['invoice_phone'] = vagueMobile($val['invoice_phone']);
            }
       }
    ...
    public function doAddInvoice() {
        ...
        $params['invoice_phone'] = encrypt($params['invoice_phone']);
	 ...
	 * @date 2015-08-20
	*/
	public function doDelInvoice(){
        $module = M('InvoiceCollect',C('DB_PREFIX'),'DB_CUSTOM');
        $invoice_id = $this->_get('id');
		if(empty($invoice_id)){
			$invoice_id = $this->_post('id');
		}
        $member = session("Members");
        if(!empty($member['m_id'])){
			if(is_array($invoice_id)){
				//$invoice_id = implode( ',', array_values($invoice_id));
				foreach($invoice_id as $key=>$val){
					$ary_res = $module->where(array("m_id"=>$member['m_id'],"id"=>$val))->delete();
				}
				$this->success('删除成功！', array(L('BACK') => U('Ucenter/Collect/pageList')));
			}else{
				$ary_res = $module->where(array("m_id"=>$member['m_id'],"id"=>$invoice_id))->delete();
			}
            if(false !== $ary_res){
    ...
    public function sendEmailCode() {
        ...
        //获取注册发送验证码模板
        $ary_option = D('EmailTemplates')->sendValidateEmail($ary_member['m_password'],$ary_member['m_name'],$ary_post['m_email']);
    ...
    public function doEmailValidate() {
		...
		if($up_res){
			$this->success('邮箱验证成功', U('Ucenter/My/pageProfile'));
		}else{
			$this->error('邮箱验证失败', U('Ucenter/My/pageProfile'));
		}
    } 

    public function supplier(){//成为供应商
        $m_id = $_SESSION['Members']['m_id'];
        $Supplier = D('Supplier')->getSupplierinfo($m_id);
        if($Supplier){
	  ...
	  public function dosupplier(){
        ...
        if ($_POST) {
            ...
            //商品品牌ID
                 $data["s_gb_id"] = (!empty($this->_post('s_gb_id')))?$this->_post('s_gb_id'):0;    
                ...
                $tmp_files1=$_FILES['s_business_photo'];
                 if(isset($tmp_files1) && !empty($tmp_files1['name']) && !empty($tmp_files1['tmp_name'])){
                        $file_extend_index = 'extend_field_1';
                        $upfiles1[$file_extend_index] = $tmp_files1;
                        if(!$upload->upload($path,$upfiles1)) {// 上传错误提示错误信息
                            //$this->error($message['msg']);
                        }else{// 上传成功 获取上传文件信息
                            $info1 =  $upload->getUploadFileInfo();
                            $tmp_files_url1 = './Public/Uploads/'.CI_SN.'/home/' .date('Ymd').'/'. $info1[0]['savename'];
                            $data['s_business_photo'] = D('ViewGoods')->ReplaceItemPicReal($tmp_files_url1);
                        }
                    }
               $tmp_files2=$_FILES['s_trademark_photo'];
                 if(isset($tmp_files2) && !empty($tmp_files2['name']) && !empty($tmp_files2['tmp_name'])){
                        $file_extend_index = 'extend_field_2';
                        $upfiles2[$file_extend_index] = $tmp_files2;
                        if(!$upload->upload($path,$upfiles2)) {// 上传错误提示错误信息
                            //$this->error($message['msg']);
                        }else{// 上传成功 获取上传文件信息
                            $info2 =  $upload->getUploadFileInfo();
                            $tmp_files_url2 = './Public/Uploads/'.CI_SN.'/home/' .date('Ymd').'/'. $info2[0]['savename'];
                            $data['s_trademark_photo'] = D('ViewGoods')->ReplaceItemPicReal($tmp_files_url2);
                        }
                    }
                 $tmp_files3=$_FILES['s_tax_photo'];
                 if(isset($tmp_files3) && !empty($tmp_files3['name']) && !empty($tmp_files3['tmp_name'])){
                        $file_extend_index = 'extend_field_3';
                        $upfiles3[$file_extend_index] = $tmp_files3;
                        if(!$upload->upload($path,$upfiles3)) {// 上传错误提示错误信息
                            //$this->error($message['msg']);
                        }else{// 上传成功 获取上传文件信息
                            $info3 =  $upload->getUploadFileInfo();
                            $tmp_files_url3 = './Public/Uploads/'.CI_SN.'/home/' .date('Ymd').'/'. $info3[0]['savename'];
                            $data['s_tax_photo'] = D('ViewGoods')->ReplaceItemPicReal($tmp_files_url3);
                        }
                }
                //print_r($data);exit();
                 $result = D('Supplier')->supplierSave($data);
