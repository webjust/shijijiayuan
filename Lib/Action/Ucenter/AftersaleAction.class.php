 ...
 */
class AftersaleAction extends CommonAction {
	...
	public function pageList() {
		...
		if (isset($ary_chose['our_source_sn']) && !empty($ary_chose['our_source_sn'])) {
            ...
            $ary_afersale = M('orders_refunds',C('DB_PREFIX'),'DB_CUSTOM')
				...
				->order('or_create_time desc')->limit($obj_page->firstRow . ',' . $obj_page->listRows)->select();
        }else{
            $count = M('orders_refunds',C('DB_PREFIX'),'DB_CUSTOM')->join('fx_orders on fx_orders_refunds.o_id = fx_orders.o_id')->where($ary_where)->count();
            $obj_page = new Page($count, 10);
            $page = $obj_page->show();
            $ary_or_id = array();
		...
		foreach($ary_afersale as $k=>$v){
			...
			elseif($v['or_refund_type'] == 2){
				...
				//退运费
			elseif($v['or_refund_type'] == 3){
				switch($v['or_processing_status']){
					case 0:
						$ary_afersale[$k]['refund_status'] = '退运费中';
						break;
					case 1:
						$ary_afersale[$k]['refund_status'] = '退运费成功';
						break;
					case 2:
						$ary_afersale[$k]['refund_status'] = '退运费驳回';
						break;
					default:
						$ary_afersale[$k]['refund_status'] = ''; //没有退款
				}
		...
		//echo "<pre>";print_r($ary_afersale);exit;
	...
	public function pageAdd() {
		...
		if(empty($ary_orders)){
			$check_delivery_res=D('OrdersDelivery')->where(array('o_id' => $o_id))->find();
			if(!empty($check_delivery_res)){
				$this->error('订单已发货，请确认收货后再申请售后！');exit;
			}
		...
		if(!empty($data) && is_array($data)){
			...
			$data['content'] = D('ViewGoods')->ReplaceItemDescPicDomain($data['content']);
		}
		//$ary_reason = D('Orders')->getReason($refund_type);
		//退款 退货 理由 后台读取
...
//        foreach($ary_reason as $key=>$reason){
//			if(empty($reason)){
//				unset($ary_reason[$key]);
//			}
//		}
        /* 取出扩展自定义属性项字段 start*/
...
//          echo "<pre>";print_r($ary_data);exit;
     ...
     * @date 2014-10-29
     */
    public function doAddDelivery(){
		$ary_data['o_id']            = $this->_post('oid');
		$ary_data['delivery_price']  = $this->_post('delivery_price');
		$ary_data['delivery_reason'] = $this->_post('delivery_reason','htmlspecialchars','');
    	// $this->ajaxReturn('','申请成功!请耐心等待审核!',1);
    	$obj_refunds = D('OrdersRefunds');
		$date = date('Y-m-d H:i:s');
        
        //验证是否传递要退运费的订单号
		if (!isset($ary_data['o_id']) || empty($ary_data['o_id'])) {
			$this->error('缺少订单号');exit;
		}

        //判断退运费是否提交过（只能申请一次）
        $ary_refunds = $obj_refunds->where(array('o_id'=>$ary_data['o_id'],'or_refund_type'=>3))->select();
        if(!empty($ary_refunds)){
            $this->error('您已申请过，请耐心等待处理！');exit;
        }
		...
		$ary_refunds = array(
			'o_id'           => $ary_data['o_id'],
			'm_id'           => $_SESSION['Members']['m_id'],
			'or_money'       => sprintf('%.2f',$ary_data['delivery_price']),
			'or_refund_type' => 3,	// 退运费类型
			'or_create_time' => $date,
			'm_name'         => $_SESSION['Members']['m_name'],
			// 'or_buyer_memo'  => $ary_data['delivery_reason']
		);
		$ary_refunds['or_reason'] = $ary_data['delivery_reason'];
        $ary_refunds['or_return_sn'] = strtotime("now");
		...
		//插入退款主表
		$int_or_id = $obj_refunds->add($ary_refunds);
		if (false === $int_or_id) {
			M('', '', 'DB_CUSTOM')->rollback();
			$this->error('申请提交失败。');exit;
		}
		//自动生成售后单据编号，单据编号的规则为20130628+8位单据ID（不足8位左侧以0补全）
		$int_tmp_or_id = $int_or_id;
		$or_return_sn = date("Ymd") . sprintf('%07s',$int_tmp_or_id);
		$array_modify_data = array("or_return_sn"=>$or_return_sn);
		$mixed_result = $obj_refunds->where(array("or_id"=>$int_or_id,'or_update_time'=>date('Y-m-d H:i:s')))->save($array_modify_data);
		if(false === $mixed_result){
			M('', '', 'DB_CUSTOM')->rollback();
			$this->error('申请提交失败。');exit;
		}
		...
		if(!$res_orders){
			M('', '', 'DB_CUSTOM')->rollback();
			$this->error('申请退运费失败');exit;
		}
		//事务提交
		M('', '', 'DB_CUSTOM')->commit();
        $this->success("退运费请求提交成功。");
    }
	
    public function test(){
        //print_r($_FILES);exit;
        //$ary_post = $this->_post();
        //$this->success("请求提交成功",'',true);
