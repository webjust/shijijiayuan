...
//                        $ary_result[$keydata]['orders'][$keyorder]['goods'] = array();
    ...
    public function doAuthTaobao() {
        ...
        //echo "<pre>";print_r($ary_token);exit;
        $obj_api = Apis::factory($str_platform, $ary_token);
        $str_shop_result = $obj_api->getShopInfo(array('nick' => $ary_token['taobao_user_nick']));
                            ...
                            foreach ($ary_val['orders'] as $ary_order) {
                                ...
                                if (empty($result)) {
                                    ...
                                    $set_data = array(
                                        ...
                                        'sku_properties_name' => $ary_order['sku_properties_name'],
                                        'toi_url' => $ary_order['url'],
...
//        echo "<pre>";print_r($shopCart);exit;
    ...
    public function doAddTrdorders() {
        ...
        $ary_orders['o_cost_freight'] = sprintf("%0.2f", $logistic->getLogisticPrice($ary_orders['lt_id'], $priceData));
        //商品总价
    ...
    public function batchAddTrdOrder() {
        ...
        if (!empty($ary_post['order']) && is_array($ary_post['order'])) {
            foreach ($ary_post['order'] as $tt_id => &$order) {
				...
				$logistic_price = D('Logistic')->getLogisticPrice($ary_logistics['logistics'], $ary_tmp_cart);
				//判断会员等级是否包邮
				if(isset($User_Grade['ml_free_shipping']) && $User_Grade['ml_free_shipping'] == 1){
					$logistic_price = 0;
				}
    ...
    public function thdCartList() {
        ...
        if (!empty($ary_member['m_id'])) {
            ...
            if (is_array($ary_pdt) && !empty($ary_pdt)) {
                $ary_cart_data = $Cart->getProductInfo($ary_pdt, $ary_price_data['all_pdt_price']);
            }
            $ary_cart = array();
...
//                        echo "<pre>";print_r($arr_products);
            ...
            if (is_array($ary_db_carts) && !empty($ary_db_carts)) {
                $ary_cart_res = D('Cart')->getProductInfo($ary_db_carts, $ary_price_data['all_pdt_price']);
            }
...
//            echo "<pre>";print_r($pro_datas[$pro_data[$int_pdt_id]['pmn_id']]['products'][$int_pdt_id]['rule_info']['name']);exit;
    ...
    public function thdpageAdd() {
		...
		//echo "<pre>";print_r($city_region_data);die;
        $ary_thdaddr = array(
            'cr_id' => $city_region_data[3],
            ...
            'ra_phone' => $ary_result['to_receiver_phone'],
            'ra_mobile_phone' => $ary_result['to_receiver_mobile'],
            'ra_is_default' => 1,
            'ra_status' => 1,
        ...
        $ary_data ['ary_product_data'] = D('Cart')->getProductInfo($ary_cart, $ary_price_data['all_pdt_price']);
        // 获得赠送积分
    ...
    public function ChangeLogistic() {
        ...
        if(!empty($lt_expressions['logistics_configure']) && $ary_return ['all_price'] >= $lt_expressions['logistics_configure']){
            ...
            // 购买货品的总价
            $logistic_price = D('Logistic')->getLogisticPrice($data ['lt_id'], $ary_tmp_cart,$ary_member);
        }
        $ary_return ['status'] = 1;
	...
	*/
	public function editReceiveAddress() {
		...
		if(FALSE !== $result){
			$this->ajaxReturn(array('status'=>true, 'msg'=>'修改收货地址成功！'));
		}else{
			$this->ajaxReturn(array('status'=>false, 'msg'=>'修改收货地址失败！'));
		}
	}
	...
	*/
	public function updateMemo() {
        ...
        if("SynchronousFlag" != $ary_post['identify']){
            $where = array();
	...
	*/
    ...
    public function batchGetAvailableLogisticsList() {
		...
		if ($cid < 1) {
			...
			//核对物流匹配
			$this->checkIsBatch($cid, $ary_post['lt_id']);
            $ary_available_logistics = $city->getAvailableListById($cid);
            if (count($ary_available_logistics) < 1) {
                $ary_res['success'] = 3;
                $ary_res['msg'] = '暂无匹配的物流公司';
                ...
                foreach ($ary_available_logistics as $ary_logistics) {
					if($ary_logistics['lt_id'] == $ary_post['lt_id']){
						$ary_goods_configure = $ary_post['goods_info'];
						$fl_logistics_cost = $city->getActualLogisticsFreight($ary_logistics['lt_expressions'], $ary_goods_configure['pdt_num'], $ary_goods_configure['pdt_weight'], $ary_goods_configure['pdt_price']);
                        //判断会员等级是否包邮
                        if(isset($User_Grade['ml_free_shipping']) && $User_Grade['ml_free_shipping'] == 1){
	...
	*/
	 ...
	 * @date 2014-4-26
	*/
	private function batch($cid, $lt_id) {
		$ary_region = array();
		$ary_region_id = D("RelatedLogisticCity")->field("cr_id")->where(array('lt_id'=>$lt_id))->select();
