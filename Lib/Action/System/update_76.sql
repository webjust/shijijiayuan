...
CREATE TABLE `fx_related_goodscategory_brand` (
  ...
  `gc_id` int(11) NOT NULL DEFAULT '0' COMMENT '类目ID',
  PRIMARY KEY (`rgcb_id`),
  UNIQUE KEY `gcb` (`gb_id`,`gc_id`) COMMENT '类目和品牌ID'
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='类目和品牌关联表';
...
ALTER TABLE `fx_orders`	ADD INDEX `o_pay_status` (`o_pay_status`) COMMENT '支付方式';
...
ALTER TABLE `fx_orders`	ADD KEY `o_crate_time` (`o_create_time`) COMMENT '新增时间加索引';
