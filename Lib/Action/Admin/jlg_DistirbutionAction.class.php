...
class DistirbutionAction extends AdminAction{
    			...
    			if($ary_taobao_items['status'] === true){
    				...
    				foreach($ary_items as $key=>$ary_item){
    					$ary_shop_items[$key] = array(
    						...
    						'thd_goods_picture'=>$ary_item['pic_url'],
    ...
    public function getTopItemDetailInfo($int_top_itemid,$shop_source,$is_down){
    	...
    	try{
    		...
    		try{
    			...
    			//验证商品是否已经同步到本地
    			$ary_check_exist = array('taobao_id' => $ary_top_info['data']['outer_id']);
    			$mixed_check_result = D('Goods')->where($ary_check_exist)->find();
				if(is_array($mixed_check_result) && !empty($mixed_check_result)>0){
				    //继续更新分销商品信息到本地
    ...
    public function convertTopItemsToLocal($ary_top_items, $convert_rules, $str_thd_shopsid, $top_obj = NULL) {
    	...
    	//验证商品是否已经同步到本地
    	$ary_check_exist = array('taobao_id' => $ary_top_items['outer_id']);
    	$mixed_check_result = D('Goods')->where($ary_check_exist)->find();
    	if (is_array($mixed_check_result) && count($mixed_check_result) > 0) {
			...
			//$ary_add_goods ['gt_id'] = $this->getDeafultTopType($mixd_goods_id);
			//新增商品信息（商品主表和商品明细表）
			$ary_goods = $ary_add_goods;
			$ary_goods_info = $ary_add_goods;
			$update_good_res = D('GyFx')->update('goods',array('g_id'=>$mixd_goods_id),$ary_goods);
			if (! $update_good_res) {
				// 新增top商品到本地时失败了。。。。
				return array (
						'status' => false,
				...
				//新增商品明细表
				$result_id = D('GyFx')->update('goods_info',array('g_id'=>$mixd_goods_id),$ary_goods_info);
				if(!$result_id){
					return array (
							'status' => false,
							'err_code' => 8103,
     ...
     * @date 2013-10-28
     */
    public function getDeafultTopType($int_goods_id) {
      if(!empty($int_goods_id)){
		$gt_id = D('Goods')->where(array('g_id'=>$int_goods_id))->getField('gt_id');
		if(!empty($gt_id)){
			return $gt_id;
		}    
	}    
    ...
    public function convertTopItemProv($str_property = '', $convert_rules = array(), $int_goods_id = 0, $int_top_cid = 0 , $property_alias='',$str_thd_shopsid,$props_name = '',$properties_name) {
    	...
    	foreach ($ary_tmp_pidvid as $pidvid) {
    		...
    		$gt_id = $this->getDeafultTopType($int_goods_id);
...
//     			}
    ...
    public function deliveryTemplateList(){
    	...
    	$ary_shops =  D('Gyfx')->selectAll('thd_shops'); 
    	$this->assign("page", $page);
    	$this->assign('ary_shops',$ary_shops);
    	$this->assign('ary_data',$ary_data);
