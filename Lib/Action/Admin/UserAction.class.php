...
class UserAction extends GyfxAction {
    ...
    public function index() {
        ...
        $this->redirect(U('Csftbbgy764/User/pageLogin'));
    }
    ...
    public function pageLogin() {
        if($_SERVER['SERVER_NAME']!='csftbbgy764.caizhuangguoji.com'){
    ...
    public function doLogout() {
        if(isset($_SESSION[C('USER_AUTH_KEY')])){
            ...
            $this->success('登出成功！', U('Admin/User/pageLogin'));
        }else{
            $this->error("已经登出！",U('Admin/User/pageLogin'));
        }
//        session('Admin', NULL);
//        $this->success('用户退出成功', U('Admin/User/pageLogin'));
    }
    ...
    public function doLogin() {
		...
		*/
        ...
        if (empty($auth_info)) {
            ...
            //保存登录信息
            $admin = M(C('USER_AUTH_MODEL'),C('DB_PREFIX'),'DB_CUSTOM');
            $ip = get_client_ip();
