...
class GoodsAction extends AdminAction{
	...
	public function goodsAdd(){
		...
		//获取商品类型并传递到模板
		$array_type = D("GoodsType")->where(array("gt_status"=>1,'gt_type'=>0))->select();
		...
		//获取商品品牌并传递到模板
		$this->assign("array_brand",D("GoodsBrand")->where(array("gb_status"=>1))->order('gb_order desc')->select());
	...
	public function doGoodsAdd(){
		//var_dump($_POST["goods_unsales_spec"]);
		...
		//}
		//商品折扣率
		if(isset($_POST["goods_info"]['g_discount']) &&($_POST["goods_info"]['g_discount']<= 0 || $_POST["goods_info"]['g_discount']> 1)){
			$this->errorMsg("商品折扣率必须大于0小于等于1！");
		}

		//跨境贸易
		if("1" == $_POST["goods_info"]["trade_type"]){
			if($_POST["goods_info"]["g_tax_rate"]==''){
				$this->errorMsg("商品税率不能为空。");
			}
			if($_POST["goods_info"]["g_tax_rate"] <=0 || $_POST["goods_info"]["g_tax_rate"] >1 ){
				$this->errorMsg("商品税率不正确。");
			}
		}
		...
		if($_POST["related_goods_category"][0]<10){
			$p_g_sn = 'cz00'.$_POST["related_goods_category"][0];
		}
		elseif($_POST["related_goods_category"][0]>9&&$_POST["related_goods_category"][0]<100){
			$p_g_sn = 'cz0'.$_POST["related_goods_category"][0];
		}
		elseif($_POST["related_goods_category"][0]>99){
			$p_g_sn = 'cz'.$_POST["related_goods_category"][0];
		}
		$_POST["goods_info"]['g_sn'] = $p_g_sn.rand(100000,999999);
		...
		if(isset($_POST["goods_products"]["pdt_sn"]) && !empty($_POST["goods_products"]["pdt_sn"])){
			...
			$pdt_bar_code = $_POST["goods_products"]["pdt_bar_code"];
			$NcadCode = $_POST["goods_products"]["NcadCode"];
			$HSCode = $_POST["goods_products"]["HSCode"];
			$pdt_website_url = $_POST["goods_products"]["pdt_website_url"];
			$pdt_g_remark = $_POST["goods_products"]["pdt_g_remark"];
			// 价格区间
			$pdt_up_price = $_POST["goods_products"]["up_price"];
			$pdt_down_price = $_POST["goods_products"]["down_price"];
			$str_spec_vids = $_POST["goods_products"]["spec_vids"];
//			$pdt_sale_discount  = $_POST["goods_products"]["pdt_sale_discount"];
			foreach($product_sn as $key => $pdt_sn){
				//验证货号是否输入
				$pdt_sn = $_POST["goods_info"]['g_sn'].'_'.$key;
				$_POST["goods_products"]["pdt_sn"][$key] = $pdt_sn;
				if(!$pdt_sn || "" == $pdt_sn){
				...
				if(is_array($ary_pdtsn_check) && !empty($ary_pdtsn_check)){
					$pdt_sn = $p_g_sn.rand(100000,999999).'_'.$key;
					$_POST["goods_products"]["pdt_sn"][$key] = $pdt_sn;
					$ary_pdtsn_check = D("GoodsProductsTable")->where(array("pdt_sn"=>$pdt_sn))->find();
					if(is_array($ary_pdtsn_check) && !empty($ary_pdtsn_check)){
        ...
        //商品排序
        $array_fx_goods["g_order"] = (isset($ary_goods["g_order"]) && is_numeric($ary_goods["g_order"]))?$ary_goods["g_order"]:0;
		...
		$array_goods_info["g_bar_code"] = 0;
		$array_goods_info["g_website_url"] ='';
		$array_goods_info["g_length"] =$ary_info["g_length"];
		$array_goods_info["g_width"] =$ary_info["g_width"];
		$array_goods_info["g_height"] =$ary_info["g_height"];
		$array_goods_info["g_unit"] = (isset($ary_info["g_unit"]) && "" != $ary_info["g_unit"])?$ary_info["g_unit"]:'';
		$array_goods_info["g_cell_nums"] =$ary_info["g_cell_nums"];

		$pic_keywords = explode(" ", $ary_info["pic_keywords"]); 
		$prefix = '<img src="';
		$suffix = '"';
		$pattern = "/".$prefix."([\s\S]*?)".$suffix."/";
		preg_match_all($pattern,$ary_info["g_desc"],$match);
		//print_r($match[1]);
		$i=0;
		foreach($match[1] as $key=>$val){
		    $find = 'src="'.$val.'"';
		    //echo $find."<br>";
		    if(empty($pic_keywords[$i])){
		        $i=0;
		    }
		    $replace = $find.' title="'.$pic_keywords[$i].'_'.$key.'.jpg" alt="'.$pic_keywords[$i].'_'.$key.'.jpg"';
		    $ary_info["g_desc"] = str_replace($find,$replace,$ary_info["g_desc"]);
		    $i++;
		}
		

		$array_goods_info["g_desc"] = $ary_info["g_desc"];
		$array_goods_info["g_auth"] = $ary_info["g_auth"];
		$array_goods_info["g_phone_desc"] = $ary_info["g_phone_desc"];
		...
		$array_goods_info["g_picture"] = $ary_info["g_picture"];
		...
		$array_goods_info["mobile_show"] = $ary_info['mobile_show'] == 0 ? 0 : 1;
		//跨境贸易
		if($ary_info['g_tax_rate']!='' && $ary_info['trade_type']=='1'){
			$array_goods_info["g_tax_rate"]=$ary_info['g_tax_rate'];
		}
		$array_goods_info['g_desc'] = _ReplaceItemDescPicDomain($array_goods_info['g_desc']);
		$array_goods_info['g_phone_desc'] = _ReplaceItemDescPicDomain($array_goods_info['g_phone_desc']);
		$array_goods_info['g_picture'] = D('ViewGoods')->ReplaceItemPicReal($array_goods_info['g_picture']);		
		$array_goods_info["g_attr_pic"] = D('ViewGoods')->ReplaceItemPicReal($array_goods_info['g_attr_pic']);

		if(isset($pdt_sale_price)){ //没有启用规格的商品 计算 最大 最小值
			$array_goods_info["ma_price"]=max($pdt_sale_price);
			$array_goods_info["mi_price"]=min($pdt_sale_price);
		}else{
			$array_goods_info["ma_price"] = (isset($_POST['pdt_sale_price']) && is_numeric($_POST['pdt_sale_price']))? $_POST['pdt_sale_price']:0;
			$array_goods_info["mi_price"] = $array_goods_info["ma_price"];
		}
		$array_goods_info["g_discount"] = (isset($ary_info['g_discount']) && is_numeric($ary_info['g_discount']))? $ary_info['g_discount']:1 ;
			...
			foreach($_POST["goods_pictures"] as $picture){
				...
				if("" != $picture && $picture !='/'){
					...
					if($_SESSION['OSS']['GY_QN_ON'] != '1'){
						...
						$array_picture["gp_picture"] = $picture;
					}
					...
					$array_picture['gp_picture'] = D('ViewGoods')->ReplaceItemPicReal($array_picture['gp_picture']);					
			...
			foreach($_POST["goods_unsales_spec"] as $gs_id=>$spec_value){
				...
				if(is_array($spec_value)){
					foreach ($spec_value as $value) {
						$int_gsd_id = 0;
						$string_spec_value = $value;
						if(2 == $array_tmp_spec_info["gs_input_type"]){
							if($spec_value <= 0){
								//如果是select类型的扩展属性，且值小于0，则说明未设置此属性的值
								continue;
							}
							$array_tmp_spec_detail = D("GoodsSpecDetail")->where(array("gsd_id"=>$value))->find();
							$int_gsd_id = $value;
		...
		if(isset($_POST["goods_products"]["pdt_sn"]) && !empty($_POST["goods_products"]["pdt_sn"])){
			foreach($_POST["goods_products"]["pdt_sn"] as $key => $pdt_sn){
				...
				$array_products['pdt_price_up']     = (isset($pdt_up_price[$key]) && is_numeric($pdt_up_price[$key])) ? $pdt_up_price[$key] : 0.00;
				$array_products['pdt_price_down']   = (isset($pdt_down_price[$key]) && is_numeric($pdt_down_price[$key])) ? $pdt_down_price[$key] : 0.00;
				...
				$array_products["pdt_bar_code"]      = isset($pdt_bar_code[$key])?$pdt_bar_code[$key]:0;
				$array_products["NcadCode"]      = isset($NcadCode[$key])?$NcadCode[$key]:0;
				$array_products["HSCode"]      = isset($HSCode[$key])?$HSCode[$key]:0;
				$array_products["pdt_website_url"]      = isset($pdt_website_url[$key])?$pdt_website_url[$key]:'';
				$array_products["pdt_g_remark"]      = $pdt_g_remark[$key];	//商品资料初始化，SKU总库存                    =可下单库存，冻结库存为0；
...
//				$array_products["pdt_sale_discount"]   = $pdt_sale_discount[$key];
			...
			$array_product_info["pdt_price_up"]     = (isset($_POST["pdt_price_up"]) && is_numeric($_POST["pdt_price_up"])) ? $_POST['pdt_price_up'] : 0.00;		// 价格区间
			$array_product_info["pdt_price_down"]   = (isset($_POST["pdt_price_down"]) && is_numeric($_POST["pdt_price_down"])) ? $_POST['pdt_price_down'] : 0.00;	// 价格区间
			...
			$array_product_info["pdt_bar_code"]      = isset($_POST["pdt_bar_code"])?$_POST["pdt_bar_code"]:0;
			$array_product_info["NcadCode"]      = isset($_POST["NcadCode"])?$_POST["NcadCode"]:0;
			$array_product_info["HSCode"]      = isset($_POST["HSCode"])?$_POST["HSCode"]:0;
			$array_product_info["pdt_website_url"]      = isset($_POST["pdt_website_url"])?$_POST["pdt_website_url"]:'';
//			$array_product_info["pdt_sale_discount"]   = (isset($_POST["pdt_sale_discount"]) && is_numeric($_POST["pdt_sale_discount"]))?$_POST["pdt_sale_discount"]:0.00;
		...
		$string_fields = "g_id,min(`pdt_market_price`) as `g_market_price`,min(`pdt_sale_price`) as `g_price`,max(`pdt_weight`) as `g_weight`,sum(`pdt_total_stock`) as `g_stock`,`pdt_bar_code` as `g_bar_code`,`pdt_website_url` as `g_website_url`";
		 ...
		 * 1. 跳转到商品列表页
		 * 2. 继续添加
		 * 3. 继续添加同类型的文件，这种需要商品添加表单页支持
		 **/
		$jump_type = 1;
		$string_jump_url = U("Admin/Products/pageList");
		if(isset($_POST["page_jump"]) && "" != $_POST["page_jump"]){
			$jump_type = $_POST["page_jump"];
		}
		switch($jump_type){
			case 1:
				//直接跳转到商品列表页
			...
			case 2:
				//继续添加商品资料
				$string_jump_url = U("Admin/Goods/goodsAdd");
				break;
			case 3:
				//继续添加同类型的商品
				$string_jump_url = U("Admin/Goods/goodsAdd") . "?gt_id=" . $array_fx_goods["gt_id"];
				break;
		}
		$log = new ILog('db');  
		$log->write('operation',array("管理员:".$_SESSION['admin_name'],"新增商品成功",'新增的商品为'.$array_goods_info['g_name'].',商品ID为:'.$int_goods_id));
	...
	public function goodsEdit(){
		...
		//dump($array_goods_info['g_desc']);
		$array_goods_info['g_desc']= D('ViewGoods')->ReplaceItemDescPicDomain($array_goods_info['g_desc']);
		$array_goods_info['g_phone_desc']= D('ViewGoods')->ReplaceItemDescPicDomain($array_goods_info['g_phone_desc']);
		//dump($array_goods_info['g_desc']);die();
		...
		if(!empty($_SESSION['OSS']['GY_OSS_PIC_URL']) || (!empty($_SESSION['OSS']['GY_OTHER_IP']) && !empty($_SESSION['OSS']['GY_OTHER_ON']) )){
			$domain_host=C('DOMAIN_HOST').'/';
			if("" == $array_goods_info["g_picture"] || !check_file_exists($domain_host . ltrim($array_goods_info["g_picture"],'/'))){
			...
			if("" == $array_goods_info["g_picture"] || (!file_exists(APP_PATH . ltrim($array_goods_info["g_picture"],'/')) && strpos($array_goods_info["g_picture"],'https://')<0)){
				$array_images[0]["g_picture"] = '/Public/Admin/images/product_image_index.png';
				$array_images[0]["input_value"] = '';
			}else{
				if($_SESSION['OSS']['GY_QN_ON'] == '1'){
					$array_images[0]["g_picture"] = D('QnPic')->picToQn($array_goods_info["g_picture"]);
					$array_images[0]["input_value"] = D('QnPic')->picToQn($array_goods_info["g_picture"]);
				}else{
					$array_images[0]["g_picture"] = D('QnPic')->picToQn('/' . ltrim(trim($array_goods_info["g_picture"]),'/'));
					$array_images[0]["input_value"] =  D('QnPic')->picToQn('/' . ltrim(trim($array_goods_info["g_picture"]),'/'));
				}
			}
		}
		...
		foreach($array_tmp_images as $key => $val){
			...
			$val["gp_picture"] = D('QnPic')->picToQn($val["gp_picture"]);
				...
				foreach($array_sale_product as $pid => $pname){
					foreach($array_info as $k_pid => $detail_info){
						...
						if($k_pid == $pid){
							...
							if(888 == $pid || $pname['gs_name'] == '颜色分类'){
								$detail_values = D("GoodsSpecDetail")->where(array('gsd_id'=>$detail_info["gsd_id"]))->find();
								if(!array_search($detail_values["gsd_id"],$array_system_color_spec)){
									$array_system_color_spec[$detail_info["gsd_id"]]=$detail_values;
								}
        ...
        foreach($array_unsale_spec as $key => $val){
			...
			if($val["gs_id"]!=911){//不是攻效
				...
				$array_tmp_value = D("RelatedGoodsSpec")->where($array_tmp_cond)->select();
				foreach ($array_tmp_value as $k => $value) {
					$array_unsale_spec[$key]["gsd_id"][$k] = $value["gsd_id"];
					$array_unsale_spec[$key]["gsd_aliases"][$k] = $value["gsd_aliases"];
				}
			}
		...
		//去除重复的颜色属性
        foreach($array_products as $k => $v){
            //$array_cond = array("g_id"=>$int_g_id,"gs_is_sale_spec"=>1,'pdt_id'=>$v['pdt_id'],'gs_id'=>888);
            $array_cond = array("g_id"=>$int_g_id,"gs_is_sale_spec"=>1,'pdt_id'=>$v['pdt_id']);			
			$tmp_system_color_spec = D("RelatedGoodsSpec")->where($array_cond)->find();
			$is_have_spec_color = 0;
			foreach($array_system_color_spec as $tmp_color_spec){
				if($tmp_system_color_spec['gs_id'] == $tmp_color_spec['gs_id'] && $tmp_system_color_spec['gsd_id'] == $tmp_color_spec['gsd_id']){
					$is_have_spec_color = 1;
					if(array_key_exists($tmp_system_color_spec["gsd_id"],$array_system_color_spec)){
						$array_system_color_spec[$tmp_system_color_spec["gsd_id"]]=$tmp_system_color_spec;
					}
				}				
			}
			/*if($is_have_spec_color == 0){
				 $array_system_color_spec[] = $tmp_system_color_spec;
			}*/
        }
//        dump($array_result);die;
	...
	public function doGoodsEdit(){
		...
		//商品折扣率
		if(isset($array_goods_info['g_discount']) &&($array_goods_info['g_discount']<= 0 ||$array_goods_info['g_discount']> 1)){
			$this->errorMsg("商品折扣率必须大于0小于等于1！");
		}
		//商品设置--商品编辑后是否默认下架
		$sysSetting = D('SysConfig');
		$sys_config = $sysSetting->getConfigs('GY_GOODS');
		$is_on_sale = $sys_config['IS_ON_SALE']['sc_value'];
		if($is_on_sale == 1){
			$array_goods_base['g_on_sale'] = 2;
		}
		...
		//跨境贸易
		if("1" == $array_goods_info["trade_type"]){
			if($array_goods_info["g_tax_rate"]==''){
				$this->errorMsg("商品税率不能为空。");
			}
			if($array_goods_info["g_tax_rate"] <=0 || $array_goods_info["g_tax_rate"] >1 ){
				$this->errorMsg("商品税率不正确。");
			}
		}
		...
		if(preg_match("/[^a-zA-Z0-9\/\\\._-]+/",$array_goods_info['g_sn'])){
			$this->errorMsg("商品编码不符合要求(	字母、数字或“_”、“-”、“.”、“/”、“\”组成)！");
		}
		...
		$ary_goods_info['g_desc'] = _ReplaceItemDescPicDomain($array_goods_info['g_desc']);
		$array_goods_info['g_desc'] = $ary_goods_info['g_desc'];
		$ary_goods_info['g_phone_desc'] = _ReplaceItemDescPicDomain($array_goods_info['g_phone_desc']);
		$array_goods_info['g_phone_desc'] = $ary_goods_info['g_phone_desc'];
		$array_goods_info['g_picture'] = D('ViewGoods')->ReplaceItemPicReal($array_goods_info['g_picture']);	
		if($array_goods_info['g_picture']!=''){
			$ary_goods_info['g_picture'] = $array_goods_info['g_picture'];
		}
		$ary_goods_info['g_attr_pic'] = D('ViewGoods')->ReplaceItemPicReal($ary_goods_info['g_attr_pic']);
		//跨境贸易
		if($array_goods_info['g_tax_rate']!='' && $array_goods_info['trade_type']=='1'){
			$ary_goods_info["g_tax_rate"]=$array_goods_info['g_tax_rate'];
		}else{
			$ary_goods_info["g_tax_rate"]=0;
		}

		if(isset($_POST["goods_products"]["pdt_sn"]) && !empty($_POST["goods_products"]["pdt_sn"])){
			$pdt_sale_price = $_POST["goods_products"]["pdt_sale_price"];
			$ary_goods_info["ma_price"]= max($pdt_sale_price);
			$ary_goods_info["mi_price"]= min($pdt_sale_price);
		}else{
			$ary_goods_info["ma_price"] = (isset($_POST['pdt_sale_price']) && is_numeric($_POST['pdt_sale_price']))? $_POST['pdt_sale_price']:0;
			$ary_goods_info["mi_price"] = $ary_goods_info["ma_price"];
		}
		$ary_goods_info["g_discount"] = (isset($array_goods_info['g_discount']) && is_numeric($array_goods_info['g_discount']))? $array_goods_info['g_discount']:1 ;
		...
		if(isset($_POST["goods_unsales_spec"]) && !empty($_POST["goods_unsales_spec"])){
			...
			foreach($_POST["goods_unsales_spec"] as $pid => $values){
				...
				if($pid!=911){
					...
					if(is_array($values)){//有选择时
						D("RelatedGoodsSpec")->where($array_cond)->delete();//删除之前所选的
						foreach ($values as $k => $v) {
							$array_tmp_spec_detail = D("GoodsSpecDetail")->where(array("gsd_id"=>$v))->find();
							$int_gsd_id = $v;
							$string_spec_value = $array_tmp_spec_detail["gsd_value"];
		...
		if(isset($_POST["goods_pictures"]) && !empty($_POST["goods_pictures"])){	
			...
			foreach($_POST["goods_pictures"] as $key => $pictures){
				...
				$pictures = D('ViewGoods')->ReplaceItemPicReal($pictures);	
		...
		if(isset($_POST["goods_products"]) && !empty($_POST["goods_products"])){
			...
			$array_pdt_price_up       = $_POST["goods_products"]["up_price"];
			$array_pdt_price_down     = $_POST["goods_products"]["down_price"];
			...
			$array_pdt_bar_code         = $_POST["goods_products"]["pdt_bar_code"];
			$array_NcadCode         = $_POST["goods_products"]["NcadCode"];
			$array_HSCode         = $_POST["goods_products"]["HSCode"];
			$array_pdt_min_num        = $_POST["goods_products"]["pdt_min_num"];
			$array_pdt_g_remark        = $_POST["goods_products"]["pdt_g_remark"];
//			$array_pdt_sale_discount     = $_POST["goods_products"]["pdt_sale_discount"];
			...
			foreach($array_pdt_sn as $pdt_id => $pdt_sn){
				...
				$array_modify_info["pdt_price_up"] = (isset($array_pdt_price_up[$pdt_sn]) && is_numeric($array_pdt_price_up[$pdt_sn]))?$array_pdt_price_up[$pdt_sn]:0.00;
				$array_modify_info["pdt_price_down"] = (isset($array_pdt_price_down[$pdt_sn]) && is_numeric($array_pdt_price_down[$pdt_sn]))?$array_pdt_price_down[$pdt_sn]:0.00;
...
//				$array_modify_info["pdt_sale_discount"] = (isset($array_pdt_sale_discount[$pdt_id]) && is_numeric($array_pdt_sale_discount[$pdt_id]))?$array_pdt_sale_discount[$pdt_id]:1.00;
				$array_modify_info["pdt_weight"] = $array_pdt_weight[$pdt_id];
				$array_modify_info["pdt_bar_code"] = $array_pdt_bar_code[$pdt_id];
				$array_modify_info["NcadCode"] = $array_NcadCode[$pdt_id];
				$array_modify_info["HSCode"] = $array_HSCode[$pdt_id];
				$array_modify_info["pdt_min_num"] = $array_pdt_min_num[$pdt_id];
				$array_modify_info["pdt_g_remark"] = $array_pdt_g_remark[$pdt_id];
				...
				if(is_array($mixed_result) && !empty($mixed_result)){
					...
					if(false === D("GoodsProductsTable")->where($array_cond)->save($array_modify_info)){
						...
						D('Gyfx')->deleteOneCache('goods_products',array('pdt_sale_price','g_id'), array('pdt_id'=>$pdt_id), $ary_order=null,600);			
						D('Gyfx')->deleteOneCache('goods_products','pdt_collocation_price', array('pdt_id'=>$pdt_id), null,3600);
						...
						D('Gyfx')->deleteOneCache('goods_products','pdt_min_num', array('pdt_id'=>$pdt_id), $ary_order=null,600);
						D('Gyfx')->deleteOneCache('goods_products','g_id', array('pdt_id'=>$pdt_id), $ary_order=null,300);						
					}
				}else{
					//否则，认为是新增一个SKU
					//验证货号的唯一性
				...
				if("" != $array_spec_vids){
					...
					foreach($array_pidvid as $pidvid){
                        ...
                        if(888 == $int_pid && $_POST["color_pic"]){
							//TODO 此处保存商品规格颜色图片
							//删除要编辑的商品规格明细图片
							D("RelatedGoodsSpec")->where(array('d_id'=>$int_g_id,'pdt_id'=>$int_pdt_id,'gsd_id'=>$int_vid))->delete();
							$result = D("RelatedGoodsSpec")->add($array_related_info);												
						}
			...
			$array_modify_data["pdt_price_up"] = (isset($_POST["pdt_price_up"]) && is_numeric($_POST["pdt_price_up"]))?$_POST["pdt_price_up"]:0.00;
			$array_modify_data["pdt_price_down"] = (isset($_POST["pdt_price_down"]) && is_numeric($_POST["pdt_price_down"]))?$_POST["pdt_price_down"]:0.00;
			...
			$array_modify_data["pdt_bar_code"]      = isset($_POST["pdt_bar_code"])?$_POST["pdt_bar_code"]:0;
			$array_modify_data["NcadCode"]      = isset($_POST["NcadCode"])?$_POST["NcadCode"]:0;
			$array_modify_data["HSCode"]      = isset($_POST["HSCode"])?$_POST["HSCode"]:0;
			$array_modify_data["pdt_min_num"] = (isset($_POST["pdt_min_num"]) && is_numeric($_POST["pdt_min_num"]))?$_POST["pdt_min_num"]:0;
//			$array_modify_data["pdt_sale_discount"] = (isset($_POST["pdt_sale_discount"]) && is_numeric($_POST["pdt_sale_discount"]))?$_POST["pdt_sale_discount"]:1.00;
			...
			if(isset($_POST["product_member_level_price"]) && is_array($_POST["product_member_level_price"])){
				foreach($_POST["product_member_level_price"] as $ml_id => $price){
					...
					if(false === D("ProductMemberLevelPrice")->add($array_info,array(),true)){
						...
						D('Gyfx')->deleteOneCache('product_member_level_price','pmlp_price', array('ml_id'=>$ml_id,'pdt_id'=>$int_pdt_id, 'pmlp_status'=>1), null,600);	
					}					
				}
			}
		}
		...
		$string_fields = "g_id,min(`pdt_market_price`) as `g_market_price`,min(`pdt_sale_price`) as `g_price`,max(`pdt_weight`) as `g_weight`,sum(`pdt_total_stock`) as `g_stock`";
		$array_result = D("GoodsProductsTable")->where($array_fetch_condition)->getField($string_fields);
		if(false === $array_result){
			D("GoodsProductsTable")->rollback();
			$this->errorMsg("商品资料保存失败。CODE:GET-MAX-PRODUCT-MARKET-PRICE-FAILED;");
		...
		if($_POST['goods']['g_on_sale'] == 2){
			$previousPage .='/tabs/shelves';
		}
		$string_jump_url = U("Admin/Products/pageList"). $previousPage;
		$log = new ILog('db');  
		$log->write('operation',array("管理员:".$_SESSION['admin_name'],"商品资料保存成功。",'商品资料保存成功。为'.$ary_goods_info['g_name'].',商品ID为:'.$int_g_id));		
		D('Gyfx')->deleteOneCache('goods_info','g_id,g_name,g_description,g_keywords',array('g_id'=>$int_g_id));
		D("Gyfx")->deleteOneCache('goods_products',"min(`pdt_market_price`) as `g_market_price`,min(`pdt_sale_price`) as `g_price`", array("g_id"=>$int_g_id,"pdt_satus"=>1), $ary_order=null,300);		 
		 D('Gyfx')->deleteOneCache('goods','g_is_combination_goods,gb_id', array('g_id' => $int_g_id), $ary_order=null,$time=null);
		$this->successMsg("商品资料保存成功。",$string_jump_url);
		exit;
	}
	...
	public function ajaxLoadUnsaleSpec(){
		...
		if($_POST['def']==1){
			$this->display('./Tpl/Admin/Goods/ajaxLoadUnsaleSpecdef.html');
		}
		else{
			$this->display();
		}
    ...
    public function addCombinationGoods(){
		...
		**/
		$log = new ILog('db');  
		$log->write('operation',array("管理员:".$_SESSION['admin_name'],"组合商品添加成功",'组合商品添加成功。为'.$ary_com_goods['g_name']));	
    ...
    public function editCombinationGoods(){
        ...
        $combinationGoods->commit();
		$log = new ILog('db');  
		$log->write('operation',array("管理员:".$_SESSION['admin_name'],"组合商品修改成功",'组合商品修改:'.$ary_edit_goods['g_name']));	
    ...
    public function ajaxDelCombiantionGoods(){
        ...
        D("GoodsBase")->commit();
		$log = new ILog('db');  
		$log->write('operation',array("管理员:".$_SESSION['admin_name'],"组合商品删除成功！",'组合商品修改:'.$str_g_id));			
...
//			$array_search_cond["g_name"] = array('like','%' . $_GET["keywords"] . '%');
        ...
        foreach($array_goods as &$good){
			...
			$good['g_picture'] = D('QnPic')->picToQn($good['g_picture']);
		}
		//返回结果给页面
    ...
    public function addCombinationPropertyGoods(){
		...
		$log = new ILog('db');  
		$log->write('operation',array("管理员:".$_SESSION['admin_name'],"执行添加规格组合商品",'执行添加规格组合商品:'.$array_data['com_name']));		
    ...
    public function editCombinationPropertyGoods(){
        ...
        $rel_spec_combination->commit();
		$log = new ILog('db');  
		$log->write('operation',array("管理员:".$_SESSION['admin_name'],"执行编辑规格组合商品",'执行编辑规格组合商品:'.$array_edit_com['com_name']));		
        $this->ajaxReturn(array('status'=>'success','msg'=>'操作成功'));
    }
    ...
    public function ajaxDelCombinationPropertyGoods(){
		...
		$log = new ILog('db');  
		$log->write('operation',array("管理员:".$_SESSION['admin_name'],"执行批量删除规格数组商品",'要删除的规格组合商品:'.$str_scg_id));			
    ...
    public function doBatchAddGoods(){
        ...
        for($row=2; $row <= $highestRow; $row++){
            ...
            $array_goods_info[$i]['good_picture'] = D('ViewGoods')->ReplaceItemPicReal($good_picture);
			...
			//手机端商品详情资料
            $g_phone_desc = trim($objPHPExcel->getActiveSheet()->getCell('AH' . $row)->getCalculatedValue());
            if(isset($g_phone_desc)){
                $array_goods_info[$i]['g_phone_desc']  = $g_phone_desc;
            }
            $i++;
        }
        ...
        foreach ($array_goods_info as $good_key=>$good_val){
            ...
            $array_good_info["g_description"] = _ReplaceItemDescPicDomain($array_good_info["g_description"]);
            ...
            $array_good_info["g_cell_nums"] = $ary_goods["g_cell_nums"];
            ...
            $array_good_info["g_picture"] = D('ViewGoods')->ReplaceItemPicReal($array_good_info["g_picture"]);
			...
			$array_good_info["g_phone_desc"] = $ary_goods["g_phone_desc"];
            ...
            if(isset($ary_goods["pics"]) && !empty($ary_goods["pics"])){
                foreach($ary_goods["pics"] as $picture){
                    ...
                    if("" != $picture){
                        ...
                        $array_picture["g_id"] = $int_goods_id;
                        $array_picture["gp_picture"] = '/'.str_replace("//","/",ltrim(str_replace('Lib/ueditor/php/../../../','',$picture),'/'));
                        $array_picture["g_picture"] = D('ViewGoods')->ReplaceItemPicReal($array_picture["g_picture"]);
						$array_picture["gp_order"] = $i;
                        $array_picture["gp_status"] = 1;
                        $array_picture["gp_create_time"] = $string_date_time;
                        $array_picture["gp_update_time"] = $string_date_time;
			...
			$log = new ILog('db');  
			$log->write('operation',array("管理员:".$_SESSION['admin_name'],"执行批量添加商品",'执行批量添加商品:'.$int_goods_id));				
        }
        M('',C('DB_PREFIX'),'DB_CUSTOM')->commit();
        //提示商品资料添加成功，并且跳转到目标页面。			
		$this->successMsg("商品资料保存成功。",U("Admin/Products/pageList"));
    ...
    public function pageSet() {
        $this->getSubNav(3, 0, 55);
        $sysSetting = D('SysConfig');
        $sys_config = $sysSetting->getConfigs('GY_GOODS');
        $this->assign($sys_config);
        $this->display();
    }
    ...
    public function doSet() {
        $ary_post = $this->_post();
        if(empty($ary_post)) {
            $this->error('请求参数为空！', U('Admin/Goods/pageSet'));
        }
        $ary_desc = array(
            'VARIABLE_DEPTH' => '商品前台显示销售属性维度',
			'IS_ON_SALE' => '商品编辑后是否默认下架'
        );
        $SysSeting = D('SysConfig');
        $SysSeting->startTrans();
        $status = true;

        foreach($ary_post as $sc_key => $sc_value) {
            $res = $SysSeting->setConfig('GY_GOODS', strtoupper($sc_key), $sc_value, $ary_desc[$sc_key]);
            if($res === false) {
                $status = false;
                break;
            }
        ...
        if($status) {
            $SysSeting->commit();
            $this->success('设置保存成功！', U('Admin/Goods/pageSet'));
        }
        else {
            $SysSeting->rollback();
            $this->error('设置保存失败！', U('Admin/Goods/pageSet'));
        }
