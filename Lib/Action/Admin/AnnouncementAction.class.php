...
class AnnouncementAction extends AdminAction{
    ...
    public function index(){
        $this->redirect(U('Admin/Announcement/pageList'));
    }
    /**
     * 违规公告列表
     * @author lf <liufeng@guanyisoft.com>
    ...
    public function pageList(){
    	$noticeObj = M('announcement',C('DB_PREFIX'),'DB_CUSTOM');
    ...
    public function doAdd(){
		...
		$data['pn_content'] = _ReplaceItemDescPicDomain($data['pn_content']);//描述显示
		...
		if($notice_id){
	        ...
	        if((int)$data['pn_is_all']==1){
	            //允许全部会员
	            $insert_pn_mid = array('mc_id'=>$notice_id,'m_id'=>-1,'mc_type'=>4);
	            $memberCompetenceObj->data($insert_pn_mid)->add();
	        }else{
	            //指定的会员
	            foreach($data['pn_mid'] as $v){
	                $insert_pn_mid[] = array('mc_id'=>$notice_id,'m_id'=>$v,'mc_type'=>4);
	            }
	        ...
	        if((int)$data['pn_is_all']==0){
	            foreach($data['pn_mg'] as $v){
	                $insert_pn_mgid[] = array('mc_id'=>$notice_id,'mg_id'=>$v,'mc_type'=>4);
	            }
	        ...
	        if((int)$data['pn_is_all']==0){
	            foreach($data['pn_ml'] as $v){
	                $insert_pn_mlid[] = array('mc_id'=>$notice_id,'ml_id'=>$v,'mc_type'=>4);
	            }
	            $memberCompetenceObj->addAll($insert_pn_mlid);
	        }
			$this->log->write('operation',array("管理员:".$_SESSION['admin_name'],"添加网站违规公告成功",'添加的网站违规公告为：'.$data['pn_title']));			
			$this->success('操作成功',U('Admin/Announcement/pageList'));
		}else{
			$this->error($noticeObj->getDbError());
		}
    }
    /**
     * 编辑违规公告
    ...
    public function pageEdit(){
		...
		$noticeInfo['pn_content'] = D('ViewGoods')->ReplaceItemDescPicDomain($noticeInfo['pn_content']);//描述显示
    	$this->assign('notice',$noticeInfo);
    	$mcObj = M('member_competence',C('DB_PREFIX'),'DB_CUSTOM');
    	$mcList = $mcObj->where(array('mc_type'=>'4','mc_id'=>$pnid))->select();
    	$myMember = array();
    	$myGroups = array();
    	$myLevels = array();
    ...
    public function doEdit(){
		...
		$data['pn_content'] = _ReplaceItemDescPicDomain($data['pn_content']);//描述显示		
    	$result = $noticeObj->where(array("pn_id"=>$pn_id))->save($data);
		if($result){
			...
			$this->log->write('operation',array("管理员:".$_SESSION['admin_name'],"编辑网站违规公告成功",'编辑的网站违规公告为：'.$data['pn_title']));				
			$this->success('操作成功',U('Admin/Announcement/pageList'));
		}else{
			$this->error($noticeObj->getDbError());
		}
    ...
    public function doDel(){
		...
		$this->log->write('operation',array("管理员:".$_SESSION['admin_name'],"删除网站违规公告成功",'删除的网站违规公告为：'.$pnid.'-'.$pn_title));		
		$this->success('操作成功',U('Admin/Announcement/pageList'));
    }
