...
class DecryptionAction extends HomeAction {
        ...
        public function bulkUpdate() {
            ...
            while(! feof($file))
            {
                ...
                if($csv_line[0]){
                    $url = 'http://'.$csv_line[0].'/Home/Decryption/index';
                    unset($csv_line);
                    echo $url.'<br/>';
                    //continue;
                    $ch = curl_init();
    ...
    public function index() {
        set_time_limit(0);
        $this->decryptAllCustomerMobile();
        $this->decryptAllDeliveryMobile();
        $this->decryptAllOrdersMobile();
        $this->decryptAllReceiveAddressMobile();
        $this->decryptAllShippingAddressMobile();
        $this->decryptAllThdOrdersMobile();
        $this->decryptAllMembersVerifyMobile();
        $this->decryptAllInvoiceCollectMobile();
    }

    public function decryptAllCustomerMobile() {
        ...
        foreach($members as $member) {
            ...
            $save_data = array();
            //���ܴ洢�ֻ�
            $mobile =  strpos($mobile, ':') ? decrypt($mobile) : '';

            if($mobile && strpos($mobile, ':')) {
                $save_data['m_mobile'] = $mobile;
                writeLog($member['m_id'].",".$mobile.','.$save_data['m_mobile'],'encodeAllCustomerMobile_1.log');
            }elseif(!strpos($mobile, ':')){
                unset($save_data['m_mobile']);
            }
            //���ܴ洢�̻�
            $telephone =  strpos($telephone, ':') ? decrypt($telephone) : '';
            if($telephone && !strpos($telephone, ':')) {
                $save_data['m_telphone'] = ($telephone);
                ...
                */
            }elseif(!strpos($telephone, ':')){
                unset($save_data['m_telphone']);
            }
    ...
    public function decryptAllDeliveryMobile() {
        ...
        foreach($orders_delivery as $od) {
            ...
            $mobile =  strpos($mobile, ':') ? decrypt($mobile) : '';
            if($mobile && strpos($mobile, ':')) {
                $save_data['od_receiver_mobile'] = $mobile;
                writeLog($od['od_id'].",".$od['o_id'].",".$mobile.','.$save_data['od_receiver_mobile'],'encodeAllDeliveryMobile_1.log');
            }elseif(!strpos($mobile, ':')){
                unset($save_data['od_receiver_mobile']);
            }
            //���ܴ洢�̻�
            $telephone =  strpos($telephone, ':') ? decrypt($telephone) : '';
            if($telephone && strpos($telephone, ':')) {
                $save_data['od_receiver_telphone'] = ($telephone);
                ...
                */
                ...
                unset($save_data['od_receiver_telphone']);
            }
    ...
    public function decryptAllOrdersMobile() {
        ...
        foreach($orders as $order) {
            ...
            $mobile =  strpos($mobile, ':') ? decrypt($mobile) : '';
            if($mobile && strpos($mobile, ':')) {
                $save_data['o_receiver_mobile'] = $mobile;
                ...
                */
                ...
                unset($save_data['o_receiver_mobile']);
            }

            //���ܴ洢�̻�
            $telephone =  strpos($telephone, ':') ? decrypt($telephone) : '';
            if($telephone && strpos($telephone, ':')) {
                $save_data['o_receiver_telphone'] = ($telephone);
                ...
                */
                ...
                unset($save_data['o_receiver_telphone']);
            }
    ...
    public function decryptAllReceiveAddressMobile() {
        ...
        foreach($receiveAddress as $address) {
            ...
            $mobile =  strpos($mobile, ':') ? decrypt($mobile) : '';
            if($mobile && strpos($mobile, ':')) {
                $save_data['ra_mobile_phone'] = $mobile;
                ...
                */
                ...
                unset($save_data['ra_mobile_phone']);
            }

            //���ܴ洢�̻�
            $telephone =  strpos($telephone, ':') ? decrypt($telephone) : '';
            if($telephone && strpos($telephone, ':')) {
                $save_data['ra_phone'] = ($telephone);
    ...
    public function decryptAllShippingAddressMobile() {
        ...
        foreach($hippingAddress as $address) {
            ...
            $mobile =  strpos($mobile, ':') ? decrypt($mobile) : '';
            if($mobile && strpos($mobile, ':')) {
                $save_data['sh_mobile_phone'] = $mobile;
                ...
                */
                ...
                unset($save_data['sh_mobile_phone']);
            }

            //���ܴ洢�̻�
            $telephone =  strpos($telephone, ':') ? decrypt($telephone) : '';
            if($telephone && strpos($telephone, ':')) {
                $save_data['sh_phone'] = ($telephone);
    ...
    public function decryptAllThdOrdersMobile() {
        ...
        foreach($thdOrders as $order) {
            ...
            $mobile =  strpos($mobile, ':') ? decrypt($mobile) : '';
            if($mobile && strpos($mobile, ':')) {
                $save_data['to_receiver_mobile'] = $mobile;
                ...
                */
                ...
                unset($save_data['to_receiver_mobile']);
            }

            //���ܴ洢�̻�
            $telephone =  strpos($telephone, ':') ? decrypt($telephone) : '';
            if($telephone && strpos($telephone, ':')) {
                $save_data['to_receiver_phone'] = ($telephone);
                ...
                */
                ...
                unset($save_data['to_receiver_phone']);
            }
    ...
    public function decryptAllMembersVerifyMobile() {
    ...
    public function decryptAllInvoiceCollectMobile() {
        ...
        foreach($invoiceCollect as $invoice) {
            ...
            $telephone =  strpos($telephone, ':') ? decrypt($telephone) : '';
            if($telephone && strpos($telephone, ':')) {
                $save_data['invoice_phone'] = $telephone;
