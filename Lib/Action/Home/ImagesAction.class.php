    ...
    public function showImage() {
		...
		if(file_exists($thumbname)){
			...
			if(!empty($_SESSION['OSS']['GY_OSS_PIC_URL']) || (!empty($_SESSION['OSS']['GY_OTHER_IP']) && !empty($_SESSION['OSS']['GY_OTHER_ON']) )){
				...
				$is_pic_exist = file_get_contents($thumbname,0,null,0,1);
				if($is_pic_exist && $is_pic_exist !='<'){
					$pic_exists=true;
			...
			mkdir(APP_PATH . $info['dirname'] . '/_thumb/');
