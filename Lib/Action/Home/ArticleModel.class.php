...
class ArticleModel extends GyfxModel {
    ...
    public function pageList($tag){
        ...
        if($memcaches->getStat() && $ary_return = $memcaches->C()->get($cache_key.CI_SN)){
            ...
            $title = isset($tag['atitle']) ? $tag['atitle'] : '';
            $cid = $tag['cid'];
            $num = $tag['num'];
            $hot = isset($tag['hot']) ? $tag['hot'] : '';
            $aid = isset($tag['aid']) ? $tag['aid'] : '';
            //print_r($tag['no_paged']);die;
            if(isset($tag['no_paged']) && $tag['no_paged'] == 1){
                unset($_GET['p']);
            }
            $tag['p'] = isset($tag['p']) ? $tag['p'] : '';
            ...
            $list = $articleOjb->field('a_id,a_title,a_create_time,hot,a_desc,hits,a_is_display,fx_article.cat_id,a_link,fx_article_cat.cat_name,ul_image_path')
    ...
    public function getArticleInfo($tag){
		...
		$list = $articleOjb->field('a_id,a_title,a_create_time,hot,a_desc,hits,a_content,fx_article.cat_id,a_link,fx_article_cat.cat_name')
						->join('fx_article_cat on fx_article.cat_id=fx_article_cat.cat_id')
						->where($where)
						->find();
        //详情页URL	aurl
    ...
    public function getCateInfo($tag) {
		...
		foreach ( $data as &$c ) {
			...
			if(!empty($tag['is_show'])){
				$c['list'] = D('Gyfx')->selectAllCache('article','a_id,a_title,a_create_time,hot,a_desc,hits,cat_id', array('cat_id'=>$c ['cat_id'],'a_status'=>1));
