 ...
 */
class NewListAction extends HomeAction{
    ...
    public function index()
    {
        ...
        if (!empty($ary_request['view']) && $ary_request['view'] == 'preview') {
            $tpl = './Public/Tpl/' . CI_SN . '/preview_' . $ary_request['dir'] . '/newlist.html';
        } else {
            $tpl = './Public/Tpl/' . CI_SN . '/' . TPL . '/newlist.html';
        }
        if($_GET['v']==2){
            ...
            $goodslist = $ApiUtil->GetFilterGoodsList($_REQUEST['cid'],$_REQUEST['cname'],$_REQUEST['bid'],$_REQUEST['func'],$_REQUEST['minPrice'],$_REQUEST['maxPrice'],$_REQUEST['keywords'],$_REQUEST['ishot'],"1");
            ...
            $tpl = FXINC . '/Public/Tpl/' . CI_SN . '/' . TPL . '/newlist-v2.html';
            $this->assign("v",'-v2');
