...
class AppUserAction extends HomeAction {  
...
//        $m_password_1 = !empty($m_password_1) ? $m_password_1 : $this->_post('m_password_1');
    ...
    */
	public function updAppUserPassword(){
		...
		//$upd_password = "123456";
		//二次密码
		$two_password = $this->_post('comf_password');
		//$two_password = "123456";
		
		//$o_password   = base64_decode($m_password);
		//$n_password   = base64_decode($upd_password);
		//$cmf_password = base64_decode($two_password);
		
		//$rand = date("YmdH");
		
		//$old_password = substr($o_password,0,$this->sunday(strval($rand),strval($o_password)));
        //$old_password = !empty($old_password) ? $old_password : $o_password;
		
        //$news_password = substr($n_password,0,$this->sunday(strval($rand),strval($n_password)));
        //$news_password = !empty($news_password) ? $news_password : $n_password;
		
		//$comf_password = substr($cmf_password,0,$this->sunday(strval($rand),strval($cmf_password)));
        //$comf_password = !empty($comf_password) ? $comf_password : $cmf_password;
		
		///实例化用户
		$member   = D("Members");
		$userData = $member->getAppUserInfo($m_id);
		
		if(empty($m_id) || empty($m_password) || empty($upd_password) || empty($two_password)){
			$result["info"]   = "参数错误";
			$result["status"] = "10001";
			print_r(json_encode($result));
			die;
    ...
    */
	public function doForgetUserPass(){
		...
		$m_password    = $this->_post('m_password');
		//二次密码
		$comf_password = $this->_post('comf_password');
		
		if(empty($m_id) || empty($m_password) || empty($comf_password)){
			$result["info"]   = "参数错误";
			$result["status"] = "10001";
			print_r(json_encode($result));
			die;
		}else{
			if($m_password != $comf_password){
				$result["info"]   = "二次密码不一致";
				$result["status"] = "10002";
				print_r(json_encode($result));
				die;
			}
			$data["m_password"] = md5($m_password);
			$regs = $member->appUpdUserInfo($m_id,$data);
...
//                echo "<pre>";print_r($mGroups);exit;
	...
	public function appIndexBrand(){
		...
		//品牌信息
		$brand = D("GoodsBrand");
		$branddata = $brand->field("gb_id,gb_logo,gb_name,gb_letter,gb_order")->where(array("gb_display"=>"1"))->order("gb_id desc")->limit(7)->select();
	...
	**/
	public function appAlipay(){
		...
		//查询订单信息
		$OrdersInfo = M("Orders")->field("o_id,o_goods_all_price,o_all_price")->find();
