...
//                alert(pdt_id);return false;
    ...
    function checkThisGood(type){
        var url = "/Ucenter/Trdorders/checkTrdCartGoods/";
        $(".addNums").unbind("click");
        $("a[pdt_sale_price]").each(function(){
            $(this).removeClass('addNums');
        });
        $.post(url,{},function(dataMsg){
            $("#li_i_reward_point").html(dataMsg.reward_point);
            $("#li_i_consume_point").html(dataMsg.consume_point);
            $("#strong_all_price").html("<i class='price'>&yen;</i>"+dataMsg.all_price);
            $("#pdt_price").html("<i class='price'>&yen;</i>"+dataMsg.all_pdt_price);
            $("#label_pre_price").html("<i class='price'>&yen;</i>"+dataMsg.pre_price);
            $("a[pdt_sale_price]").each(function(){
