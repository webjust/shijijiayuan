...
(function () {
    ...
    //阿里云OSS   
	var oss_val = document.getElementById("oss_id");
	if(oss_val==null || oss_val==undefined){
		var oss_id = '';
	}else{
		var oss_id = oss_val.value;
	}
    var image_path = '';
    if(oss_id != '1'){
    	image_path = URL + "php/";
	...
	//七牛  
	var qn_val = document.getElementById("qn_id");
	if(qn_val==null || oss_val==undefined){
		var qn_id = '';
	}else{
		var qn_id = qn_val.value;
	}
    if(qn_id == '1'){
    	image_path =  "";
    }
    ...
    window.UEDITOR_CONFIG = {
        ...
        //图片上传配置区
        ,imageRealPath:''                            //自定义的上传子目录
        ,imageUrl:URL+"php/imageUp.php"             //图片上传提交地址
        //,imagePath:URL + "php/"                     //图片修正地址，引用了fixedImagePath,如有特殊需求，可自行配置
		,imagePath:image_path                       //图片修正地址，引用了fixedImagePath,如有特殊需求，可自行配置
       //,imageFieldName:"upfile"                   //图片数据的key,若此处修改，需要在后台对应文件修改对应参数
        //,compressSide:0                            //等比压缩的基准，确定maxImageSideLength参数的参照对象。0为按照最长边，1为按照宽度，2为按照高度
        ,maxImageSideLength:1000000                    //上传图片最大允许的边长，超过会自动等比缩放,不缩放就设置一个比较大的值，更多设置在image.html中
        ...
        ,zIndex : 950     //编辑器层级的基数,默认是900
        , zIndex_DialogOver: 1000 // 弹出对话框 高于 zIndex 的层数 （ zIndex + zIndex_DialogOver 应高于所有层）
