...
$(document).ready(function(){
    ...
    /*右边购物车栏*/
    popDisplayed = false;
    $("#message_list,.ibar_closebtn").click(function(){
        if(popDisplayed==false){
            ...
            $("#quick_links_pop").animate({left:'0px'});
        }
    });
});	
...
function ajaxLoadShoppingCart(int_page_num){
    ...
    $.post('/Home/Cart/Pagelist',{'int_page_num':int_page_num},function(htmlObj){
        $(".pop_panel").html(htmlObj);
        $(".cart_price_number").html($("#cart_price").html());
        rheight();
    },'html');
}
...
/*右侧栏高度重置*/
function rheight(){
    $('.pop_panel').height($('#quick_links_pop').height()-$('.ibar_plugin_title').height());
    var cart_go_list = $('.ibar_cart_group ').height()-$('.ibar_cart_group_header').height()-90;
    $('.cart_go_list').height(cart_go_list);
}

/*右边购物车栏*/
/*popDisplayed = false;
thisaa = '';
$(".common_css a").click(function(){
    if(popDisplayed==false){
        thisaa = $(this).attr('class');
        popDisplayed = true;
        $("#quick_links_pop").animate({left:'-280px'});
    }else{
        if(thisaa==$(this).attr('class')){
            popDisplayed = false;
            $("#quick_links_pop").animate({left:'-0'});
        }
    }
    if(thisaa==$(this).attr('class')){
        $("#quick_links_pop").toggleClass("hide");
        $("#quick_links_pop").toggleClass("quick_message_list");
    }else{
        thisaa = $(this).attr('class');
    }
});*/
...
$(function(){
    ...
    $("#Lb_button li").mouseenter(function() {
        index = $("#Lb_button li").index(this);
        showPics(index);
    }).eq(0).trigger("mouseenter");
    
    $("#Lb_show").css("width",sWidth * (len));
    
    $("#Lb_show").hover(function() {
        clearInterval(picTimer);
    },function() {      
        picTimer = setInterval(function() {
            showPics(index);
            index++;
            var focusImg = $("#Lb_show li:nth-child("+index+") img");
    ...
    function showPics(index) { 
        ...
        $("#Lb_show li").css("width",sWidth);
        $("#Lb_show").stop(true,false).animate({"left":nowLeft},300); 
        $("#Lb_button li").stop(true,false).css({"background":"#E5582B","color":"black"}).eq(index).stop(true,false).css({"background":"gray","color":"white"}); 
    }
...
function getDetailSkus(item_id, item_type, v){
    $.ajax({
