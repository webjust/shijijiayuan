...
function specSelect(dom, on_class) {
    ...
    if(_class_name == on_class) {
        ...
        $(dom).siblings('a.'+on_class).attr('class','');
    }
...
function getPdtBySpecSelect(on_class) {
    ...
    $("#xdhysfl").html($(".gsdname_list").attr('data-value'));
    if(_select_all) {
        if(_gsd_arr.length > 0) {
            ...
            if (_pdt && _pdt.g_id) {
                ...
                $('#pdt_stock_xs').html(_pdt_stock);
                $('#showNum').html(_pdt_stock_show);
                $('#showPrice').length && $('#showPrice').html(_pdt_sale_price);
                $('#showPricea').length && $('#showPricea').html(_pdt_sale_price);
//                $('#surplus').length && $('#surplus').html(_pdt_stock);
    ...
    $('#pdt_stock').val(0);
    $('#pdt_stock_xs').html(0);
    $('#showPricea').html($('#showPrice').html());
    shuxingimg(0);
}
...
function shuxingimg(on_class) {
    if($(".spec_list .on img").length > 0 && on_class){
        $('#diy_gimgxs').attr("src",$(".spec_list .on img").attr("src"));
    }else{
        $('#diy_gimgxs').attr("src",$("#focus .bd img").eq(0).attr("src"));
    }
}
