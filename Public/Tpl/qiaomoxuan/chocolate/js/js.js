...
function getGoodCommentPage(gid){
    $.ajax({
        url:'/Home/Comment/getGoodCommentPage',
        dataType:'HTML',
        type:'POST',
        data:{
            gid:gid
 ...
 */
 function setGroupbuyTime(times,showDay,showHouse,showFen,showMiao,now_time,buy_type,buy_status,gpEndTime,listId,gid){
	...
	//if(buy_type == 'tuan'){
		...
		var end_time = timeStr.getTime();
		var nowTime = nowtimeStr.getTime();
		if(buy_status == '1'){
			var sys_second = (nowTime-end_time)/1000;	
		}else{
    ...
    var interval = setInterval(function(){
		if(buy_type == 'tuan'){
			...
			if(buy_status == '1'){
				if(sys_second>=0){
					...
					$('#showGroupTime'+listId).html("<span><abbr>剩余时间：</abbr></span><label id='day'>0</label>天<label id='hours'>0</label>小时<label id='minutes'>0</label>分钟<label id='seconds'>0</label>秒</div><script>setGroupbuyTime('"+gpEndTime+"','day','hours','minutes','seconds','','tuan',2);</script>");
					setGroupbuyTime(gpEndTime,'day','hours','minutes','seconds','','tuan',2);
				}
			...
			if(buy_status == '1'){
				if(sys_second>=0){
					...
					setGroupbuyTime(gpEndTime,'day','hours','minutes','seconds','','miaos',2,listId,gid);
				...
				//已开始
				if(sys_second<0){
					$('#showGrouupbuy'+listId).html('<input type="button" id="addNotOrder" disalbed class="notSpike" value="立即购买" />');
        ...
        $(day_elem).html(checkDay);
        $(hour_elem).html(showHourrs);
        $(minute_elem).html(showMinutes);
        $(second_elem).html(showSeconds);
        showSeconds = --showSeconds;
    }, 1000);
}
