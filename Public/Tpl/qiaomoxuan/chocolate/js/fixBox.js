...
$(function () {
	...
	if(is_index == 1){
		$(".efFloor-nav").fixedBox({'setEvent':'scroll','id':2,'left':0,boxT:$(".floor-wrapper").offset().top,top:0,zIndex:9});		
	}
    var arrTop = [];

    $(".floor-wrapper").find(".floor").each(function(){
        arrTop.push($(this).offset().top-300);
    });

    $(".efFloor-nav li").click(function(){
        var idx = $(this).index();
        $('body,html').animate({ scrollTop: arrTop[idx]+240 },500);
    });
	/*
    $(window).scroll(function(){
        var scrollTop = $(this).scrollTop();
        if(scrollTop > arrTop[0] && scrollTop < arrTop[1]) {
            $(".efFloor-nav li").eq(0).addClass("selected").siblings().removeClass("selected");
        }else if(scrollTop > arrTop[1] && scrollTop < arrTop[2]){
            $(".efFloor-nav li").eq(1).addClass("selected").siblings().removeClass("selected");
        }else if(scrollTop > arrTop[2] && scrollTop < arrTop[3]) {
            $(".efFloor-nav li").eq(2).addClass("selected").siblings().removeClass("selected");
        }else if(scrollTop > arrTop[3] && scrollTop < arrTop[4]) {
            $(".efFloor-nav li").eq(3).addClass("selected").siblings().removeClass("selected");
        }else if(scrollTop > arrTop[4] && scrollTop < arrTop[5]) {
            $(".efFloor-nav li").eq(4).addClass("selected").siblings().removeClass("selected");
        }else if(scrollTop > arrTop[5]) {
            $(".efFloor-nav li").eq(5).addClass("selected").siblings().removeClass("selected");
        }
    })*/
	$(window).scroll(function(){
		var scrollTop = $(this).scrollTop();
		for(var i = 0; i<arrTop.length;i++){
				if(scrollTop > arrTop[i] && (arrTop[i+1] ? (scrollTop < arrTop[i+1]):true)) {
					$(".efFloor-nav li").eq(i).addClass("selected").siblings().removeClass("selected");
				}
		}
	})
	//轮播
	...
	$(".ef-country").slide({effect:"leftLoop",autoPlay:true,trigger:"mouseover",delayTime:700,interTime:4000});
	$('.nav-category dl').last().find('b').remove();

	$(".hk-floor .col1").each(function () {
		$(this).slide({
			titCell: ".hd ul",
			mainCell: ".bd ul",
			effect: "fold",
